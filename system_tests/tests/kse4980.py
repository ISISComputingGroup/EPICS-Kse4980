...
class Kse4980Tests(unittest.TestCase):
    ...
    def test_GIVEN_invalid_function_WHEN_setting_function_THEN_function_is_not_set_and_error_id_populated(
        self,
    ):
    ...
    def test_GIVEN_reset_all_button_pressed_WHEN_non_standard_settings_applied_THEN_defaults_are_restored(
        self,
    ):
    ...
    def test_GIVEN_averaging_factor_changed_only_THEN_averaging_factor_and_measurement_time_both_updated(
        self,
    ):
        avg_fact_sp = 23
        self.ca.set_pv_value("AVG_FACTOR:SP", avg_fact_sp)
    ...
    def test_GIVEN_function_changed_THEN_correct_engineering_units_are_applied_to_readings(
        self, _, new_func, expected_first_unit, expected_second_unit
    ):
    ...
    def test_GIVEN_signallevel_set_too_high_WHEN_setting_level_THEN_error_propagates(
        self, _, signaltype, over_limit
    ):
